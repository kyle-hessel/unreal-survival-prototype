...
bool ACampCharacter::CreateCampsite_Implementation()
{
	// ****NOTE: the C++ implementation of this function is now called in the middle of its blueprint implementation. See the blueprint function CreateCampsite on BP_CampCharacter.
	if (bInCombat == false && NearbyEnemies.Num() == 0)
	{
		/* Cycle through Campsites array to determine if we can place another campsite.
		 * This COULD cause problems later if some campsites are unloaded from a far away distance.
		 * All the more reason to maybe just store campsite locations? But campsites themselves are
		 * needed as respawn points and such; yet this can be handled by InteractComp's CurrentCampsite.
		 *
		 * Also, do this check before also checking the ground around the player.
		*/
		if (Campsites.Num() > 0)
		{
			for (ACampCampsite* Campsite : Campsites)
			{
				FVector DistanceToCampsiteVector = Campsite->GetActorLocation() - GetActorLocation();
				float DistanceToCampsite = DistanceToCampsiteVector.Length();
				UE_LOG(LogTemp, Display, TEXT("Distance to campsite: %f"), DistanceToCampsite);

				// If we are too close to another campsite, return false and early out.
				if (DistanceToCampsite < MinimumCampsiteDistance)
				{
					UE_LOG(LogTemp, Warning, TEXT("Too close to another campsite."));
					return false;
				}
			}
		}
		...
		/*
		 * If we aren't too close to another campsite, check the terrain around us.
		*/
		...
		float ActorLocZOffset = GetActorLocation().Z - 130.f;
		float LateralXYOffset = 350.f;
		float DotThreshold = 0.9965f; // Should be very close to 1.0.
		...
		FVector LineTraceStart = FVector(GetActorLocation().X, GetActorLocation().Y, GetActorLocation().Z + 30.f); // These values will need tweaking depending on character size...
		FVector LineTraceEnd = FVector(GetActorLocation().X, GetActorLocation().Y, ActorLocZOffset);

		bool bBlockingHitCenter = GetWorld()->LineTraceSingleByObjectType(HitCenter, LineTraceStart, LineTraceEnd, ObjectQueryParams);
		DrawDebugLine(GetWorld(), LineTraceStart, LineTraceEnd, FColor::Purple, false, 3.0f, 0, 1.5f);

		// If center line trace was successful (ensuring player is on solid ground), do more line traces.
		if (bBlockingHitCenter)
		{
			FHitResult HitLeft;
			FVector LineTraceTwoEnd = FVector(GetActorLocation().X - LateralXYOffset, GetActorLocation().Y, ActorLocZOffset);
			bool bBlockingHitLeft = GetWorld()->LineTraceSingleByObjectType(HitLeft, LineTraceStart, LineTraceTwoEnd, ObjectQueryParams); // Left line trace
			DrawDebugLine(GetWorld(), LineTraceStart, LineTraceTwoEnd, FColor::Orange, false, 3.0f, 0, 1.5f);
		
			if (bBlockingHitLeft)
			{
				FHitResult HitRight;
				FVector LineTraceThreeEnd = FVector(GetActorLocation().X + LateralXYOffset, GetActorLocation().Y, ActorLocZOffset);
				bool bBlockingHitRight = GetWorld()->LineTraceSingleByObjectType(HitRight, LineTraceStart, LineTraceThreeEnd, ObjectQueryParams); // Right line trace
				DrawDebugLine(GetWorld(), LineTraceStart, LineTraceThreeEnd, FColor::Turquoise, false, 3.0f, 0, 1.5f);

				if (bBlockingHitRight)
				{
					FHitResult HitForward;
					FVector LineTraceFourEnd = FVector(GetActorLocation().X, GetActorLocation().Y + LateralXYOffset, ActorLocZOffset); // Forward line trace
					bool bBlockingHitForward = GetWorld()->LineTraceSingleByObjectType(HitForward, LineTraceStart, LineTraceFourEnd, ObjectQueryParams);
					DrawDebugLine(GetWorld(), LineTraceStart, LineTraceFourEnd, FColor::Cyan, false, 3.0f, 0, 1.5f);

					if (bBlockingHitForward)
					{
						FHitResult HitBackward;
						FVector LineTraceFiveEnd = FVector(GetActorLocation().X, GetActorLocation().Y - LateralXYOffset, ActorLocZOffset); // Backward line trace
						bool bBlockingHitBackward = GetWorld()->LineTraceSingleByObjectType(HitBackward, LineTraceStart, LineTraceFiveEnd, ObjectQueryParams);
						DrawDebugLine(GetWorld(), LineTraceStart, LineTraceFiveEnd, FColor::White, false, 3.0f, 0, 1.5f);

						if (bBlockingHitBackward)
						{
							// If all of our line traces succeeded, notify us and then the function will continue instead of returning false.
							UE_LOG(LogTemp, Warning, TEXT("All line traces succeeded, proceeding with dot products."));

							// Calculate averaged dot product of every HitDirection.ImpactNormal compared against player's UpVector.
							...
							float DotFour = FVector::DotProduct(GetActorUpVector(), HitForward.ImpactNormal);
							float DotFive = FVector::DotProduct(GetActorUpVector(), HitBackward.ImpactNormal);

							float DotAveraged = (DotOne + DotTwo + DotThree + DotFour + DotFive) / 5.f;
						
							UE_LOG(LogTemp, Display, TEXT(" Dot product 1: %f"), DotOne);
							UE_LOG(LogTemp, Display, TEXT(" Dot product 2: %f"), DotTwo);
							UE_LOG(LogTemp, Display, TEXT(" Dot product 3: %f"), DotThree);
							UE_LOG(LogTemp, Display, TEXT(" Dot product 4: %f"), DotFour);
							UE_LOG(LogTemp, Display, TEXT(" Dot product 5: %f"), DotFive);

							UE_LOG(LogTemp, Display, TEXT(" Dot averaged: %f"), DotAveraged);

							// If the averaged dot product is not above a certain threshold (very close to 1.0) we can infer that the terrain is too uneven in the given area.
							if (DotAveraged < DotThreshold)
							{
								UE_LOG(LogTemp, Warning, TEXT("Terrain too uneven, cannot place campsite."));
								return false;
							}

							UE_LOG(LogTemp, Warning, TEXT("Terrain relatively even, continuing with placement of campsite."));

							// If any of our line traces don't touch the ground, early out.
						} else { return false; }
		...
		{
			// If the player isn't close enough to the ground, don't spawn a campsite.
			return false;
		}
	
		// Set parameters for spawning the campsite: loc/rot, collisions, what to spawn.
	
		/**************
		 *Rotation needs modification?
		*/
		FRotator GroundAlignedRotation = UKismetMathLibrary::MakeRotFromZX(HitCenter.ImpactNormal, GetActorForwardVector());
	
		FTransform SpawnTransform = FTransform(GroundAlignedRotation, GetActorLocation());
		FActorSpawnParameters SpawnParams;
		SpawnParams.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AlwaysSpawn;
	
		SpawnedCampsite = Cast<ACampCampsite>(GetWorld()->SpawnActor<AActor>(CampsiteClass, SpawnTransform, SpawnParams));
		UE_LOG(LogTemp, Warning, TEXT("Spawned campsite!"));

		// Spawn the Campsite. We have to specify the BP_CampCampsite in the editor as the CampsiteClass to ensure we can use the blueprint version.
		if (SpawnedCampsite)
		{
			Campsites.Add(SpawnedCampsite);

			// If we create the campsite successfully, set it to be our CurrentCampsite on the InteractComp; now that we have created it, we can interact with it.
			InteractComp->SetCurrentCampsite(SpawnedCampsite);
			InteractComp->GetCurrentCampsite()->SetOwningCharacter(this);

			// Sit down at the campsite.
			SitDown();

			return true;
		}

		// If SpawnedCampsite doesn't properly cast (implying the wrong type of AActor was assigned in the editor) return false.
		return false;
	}

	UE_LOG(LogTemp, Warning, TEXT("Enemies nearby, unable to create campsite."));

	// return false if we are doing an activity that does not permit campsite creation, such as combat.
	return false;
}
